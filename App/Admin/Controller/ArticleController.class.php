    ...
    public function help(){
    	...
    	//查询条件为，parent_id为6的信息（如果没有索引查询，就查出帮助中心的所有信息）
    		...
    		foreach($article_category as $k=>$v){
    			//加入二级分类，以一级id为查询条件
    			$article_category[$k]['children']= M('Article_category')
                    ->where(C("DB_PREFIX")."article_category.parent_id = {$v['id']}")
                    ->order("id asc")->select();
    		}
...
//            $data['content'] =  html_entity_decode(I('post.content'));//内容
            ...
            $data['read_num'] =I('read_num');//是否标红
            $data['sort'] = I('sort');//排序
            $data['status'] = I('status');//排序
    ...
    private function getContentArticleByWhere($where,$Page){
    	 $info = M('Article')->join( "left join ". C('DB_PREFIX')."article_category   ON ".C('DB_PREFIX')."article.position_id = ".C('DB_PREFIX')."article_category.id")
				    ->where($where)
				    ->order(C('DB_PREFIX')."article.add_time desc,".C('DB_PREFIX')."article.sort")
                    ->field(C('DB_PREFIX')."article.*,".C('DB_PREFIX')."article_category.name,".C('DB_PREFIX')."article_category.id,".C('DB_PREFIX')."article_category.parent_id")
